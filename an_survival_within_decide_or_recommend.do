...
GenericSetupSteveHarris spot_early an_survival.do
include cr_survival.do


*  ===========================================
*  = Keep only recommended for critical care =
*  ===========================================
* keep if inlist(v_ccmds_rec,2,3)
*  =======================================
*  = Keeo only accepted to critical care =
*  =======================================
keep if inlist(v_disposal,1,2)

count if ppsample
tab v_disp if ppsample
sum time2icu if ppsample,d 
tab time2icu_cat if ppsample

stset dt1, id(id) origin(dt0) failure(dead) exit(time dt0+30)


set scheme tufte_color



* Fit now the baseline deferred model
stcox male sepsis_b rxlimits age_c ib1.icnarc_q10 defer4
* Fit the same model interacting deferred admission and severity
stcox male sepsis_b rxlimits age_c ib1.icnarc_q10##defer4

* Fit the model with icu as a tvc
stcox male sepsis_b rxlimits age_c ib1.icnarc_q10 icu
...
foreach origin of local origins {
	...
	gen icu_ever = .
	replace icu_ever = 1 if time2icu != . & _st != 0 & ppsample == 1
	replace icu_ever = 0 if time2icu == . & _st != 0 & ppsample == 1
	tab icu_ever
